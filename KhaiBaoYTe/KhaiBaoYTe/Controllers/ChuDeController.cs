...
namespace KhaiBaoYTe.Controllers
{
    public class ChuDeController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var chuDeVM = from cd in db.ChuDes
                          join t in db.Templates on cd.IDChuDe equals t.IDChuDe into gj1
                          from g1 in gj1.DefaultIfEmpty()
                          join ch in db.CauHois on g1.IDTemplate equals ch.IDTemplate into gj2
                          from gr in gj2.DefaultIfEmpty()
                          group gr by new
        ...
        public ActionResult Create([Bind(Include = "IDChuDe,TenChuDe,MoTa,NgayTao,NguoiTao,NgayUpdate")] ChuDe chuDe)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "IDChuDe,TenChuDe,MoTa,NguoiUpdate,NgayTao,NgayUpdate")] ChuDe chuDe)
        {
            if (ModelState.IsValid)
            {
                chuDe.NgayUpdate = DateTime.Now;
                db.Entry(chuDe).State = EntityState.Modified;
                db.Entry(chuDe).Property(x => x.NguoiTao).IsModified = false;
